...
public class ExplorationMap {
            ...
            if (verifyMatrix()) {
                if (matrix[row][col].getTokens().size() == 1 && matrix[row][col].getTokens().get(0).getNumber() == 0) {
                    ...
                    if (list.getTokens().size() == 0) {
                        ...
                        System.out.println("The list of tokens is empty!");
                    } else {
                        this.matrix[row][col] = list;
                        robot.setNumberOfTokens(robot.getNumberOfTokens()+list.getTokens().size());
                        System.out.println("Robot " + robot.getName() + " visited successfully the cell " + "row " + row + " column " + col );
                        return true;
                    }
                }
            }
                ...
                robot.setRunning(false);
                System.out.println("Done:" + robot.getName() + " placed " + robot.getNumberOfTokens() + " tokens ");
            }
        }
        return false;
    }
    ...
    public boolean verifyThereIsEmptyCells(){
        for(int i=0;i<rows;i++){
            for(int j=0;j<columns;j++){
                if(matrix[i][j].getTokens().size() == 1 && matrix[i][j].getTokens().get(0).getNumber() == 0){
                    return true;
